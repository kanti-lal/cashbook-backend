...
export class CustomerModel {
  static getAll(businessId, search) {
    ...
    if (search) {
      ...
      customers = db
        .prepare("SELECT * FROM customers WHERE businessId = ?")
        .all(businessId);
    }
  ...
  static delete(id, businessId) {
    const db = new Database(config.dbPath);
    db.prepare(
      "DELETE FROM transactions WHERE customerId = ? AND businessId = ?"
    ).run(id, businessId);
    db.prepare("DELETE FROM customers WHERE id = ? AND businessId = ?").run(
      id,
      businessId
    );
    db.close();
